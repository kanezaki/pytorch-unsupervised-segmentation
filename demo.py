...
class MyNet(nn.Module):
    def __init__(self, input_dim):
        ...
        self.conv1 = nn.Conv2d(
            input_dim, args.nChannel, kernel_size=3, stride=1, padding=1
        )
        ...
        for i in range(args.nConv - 1):
            self.conv2.append(
                nn.Conv2d(
                    args.nChannel, args.nChannel, kernel_size=3, stride=1, padding=1
                )
            )
            self.bn2.append(nn.BatchNorm2d(args.nChannel))
        self.conv3 = nn.Conv2d(
            args.nChannel, args.nChannel, kernel_size=1, stride=1, padding=0
        )
